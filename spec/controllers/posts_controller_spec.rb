...
describe PostsController, type: :routing do
  describe 'routing' do
    ...
    context 'not have' do
      it { should_not route(:delete, '/categories/1/1').to(action: :destroy, category_id: 1, id: 1) }
      it { should_not route(:patch, '/categories/1/1').to(action: :update, category_id: 1, id: 1) }
      it { should_not route(:put, '/categories/1/1').to(action: :update, category_id: 1, id: 1) }
      it { should_not route(:get, '/categories/1/1/edit').to(action: :edit, category_id: 1, id: 1) }
    end
  end
end
